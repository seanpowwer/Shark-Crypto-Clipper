...
Public Class tlscclient
    ...
    Sub New(ByVal CL As Socket)
        ...
        SendSync = New Object()
        If Gclass1.Blocked.Contains(IP.Split(":")(0)) Then
            isDisconnected()
            Return
        Else
    ...
    Sub isDisconnected()
        ...
        Try
            If LV IsNot Nothing Then
                If CClient2.F.l1.InvokeRequired Then
                    CClient2.F.l1.BeginInvoke(New _isDisconnected(AddressOf isDisconnected))
                    Exit Sub
                Else
                    LV.Remove()
                    log(IP.Split(":")(0), False)
                    Gclass1.Online.Remove(Me)
                End If
            End If
        Catch ex As Exception
